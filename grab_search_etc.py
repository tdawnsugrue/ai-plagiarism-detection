...
def search_and_detect(paradata, verbose = False, atire_stats = True, sherlock_stats = False, gen_index = False):
    ...
    for item in paradata[:40]:
        ...
        search_results = get_top_hits("000.txt", prompts, 5)
    ...
    print(f"Atire accuracy: {correct_count / total_count:.2%}, % ({correct_count} of {total_count})")

def atire_found_correct(data, hits):
    original = data['assistant']
    for i, h in enumerate(hits):
        if original == h : return (True, i)
    return (False, -1)
...
if __name__ == "__main__":
    ...
    f = open("output-with-paraphrasing-40.json")
